/**
 * /**
 * JUunit tests for the Scanner for the class project in COP5556 Programming Language Principles 
 * at the University of Florida, Fall 2017.
 * 
 * This software is solely for the educational benefit of students 
 * enrolled in the course during the Fall 2017 semester.  
 * 
 * This software, and any software derived from it,  may not be shared with others or posted to public web sites,
 * either during the course or afterwards.
 * 
 *  @Beverly A. Sanders, 2017
 */

package cop5556fa17;

import static org.junit.Assert.*;

import org.junit.Rule;
import org.junit.Test;
import org.junit.rules.ExpectedException;

import cop5556fa17.Scanner.LexicalException;
import cop5556fa17.Scanner.Token;

import static cop5556fa17.Scanner.Kind.*;

public class ScannerTest {

	//set Junit to be able to catch exceptions
	@Rule
	public ExpectedException thrown = ExpectedException.none();

	
	//To make it easy to print objects and turn this output on and off
	static final boolean doPrint = true;
	private void show(Object input) {
		if (doPrint) {
			System.out.println(input.toString());
		}
	}

	/**
	 *Retrieves the next token and checks that it is an EOF token. 
	 *Also checks that this was the last token.
	 *
	 * @param scanner
	 * @return the Token that was retrieved
	 */
	
	Token checkNextIsEOF(Scanner scanner) {
		Scanner.Token token = scanner.nextToken();
		assertEquals(Scanner.Kind.EOF, token.kind);
		assertFalse(scanner.hasTokens());
		return token;
	}


	/**
	 * Retrieves the next token and checks that its kind, position, length, line, and position in line
	 * match the given parameters.
	 * 
	 * @param scanner
	 * @param kind
	 * @param pos
	 * @param length
	 * @param line
	 * @param pos_in_line
	 * @return  the Token that was retrieved
	 */
	Token checkNext(Scanner scanner, Scanner.Kind kind, int pos, int length, int line, int pos_in_line) {
		Token t = scanner.nextToken();
		assertEquals(scanner.new Token(kind, pos, length, line, pos_in_line), t);
		return t;
	}

	/**
	 * Retrieves the next token and checks that its kind and length match the given
	 * parameters.  The position, line, and position in line are ignored.
	 * 
	 * @param scanner
	 * @param kind
	 * @param length
	 * @return  the Token that was retrieved
	 */
	Token check(Scanner scanner, Scanner.Kind kind, int length) {
		Token t = scanner.nextToken();
		assertEquals(kind, t.kind);
		assertEquals(length, t.length);
		return t;
	}

	/**
	 * Simple test case with a (legal) empty program
	 *   
	 * @throws LexicalException
	 */
	
	@Test
	public void testEmpty() throws LexicalException {
		String input = "";  //The input is the empty string.  This is legal
		show(input);        //Display the input 
		Scanner scanner = new Scanner(input).scan();  //Create a Scanner and initialize it
		show(scanner);   //Display the Scanner
		checkNextIsEOF(scanner);  //Check that the only token is the EOF token.
	}
	
	
	/**
	 * Test illustrating how to put a new line in the input program and how to
	 * check content of tokens.
	 * 
	 * Because we are using a Java String literal for input, we use \n for the
	 * end of line character. (We should also be able to handle \n, \r, and \r\n
	 * properly.)
	 * 
	 * Note that if we were reading the input from a file, as we will want to do 
	 * later, the end of line character would be inserted by the text editor.
	 * Showing the input will let you check your input is what you think it is.
	 * 
	 * @throws LexicalException
	 */
	
	@Test
	public void testSemi1() throws LexicalException {
		String input = ";;;;";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		checkNext(scanner, SEMI, 0,1,1,1);
		checkNext(scanner, SEMI, 1,1,1,2);
		checkNext(scanner, SEMI, 2,1,1,3);
		checkNext(scanner, SEMI, 3,1,1,4);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testSeparators() throws LexicalException {
		String input = ";,;;,";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		checkNext(scanner, SEMI, 0,1,1,1);
		checkNext(scanner, COMMA, 1,1,1,2);
		checkNext(scanner, SEMI, 2,1,1,3);
		checkNext(scanner, SEMI, 3,1,1,4);
		checkNext(scanner, COMMA, 4,1,1,5);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testEquals1() throws LexicalException {
		String input = ";==;";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		checkNext(scanner, SEMI, 0,1,1,1);
		checkNext(scanner, OP_EQ, 1,2,1,2);
		checkNext(scanner, SEMI, 3,1,1,4);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testEquals2() throws LexicalException {
		String input = ";=;";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		checkNext(scanner, SEMI, 0,1,1,1);
		checkNext(scanner, OP_ASSIGN, 1,1,1,2);
		checkNext(scanner, SEMI, 2,1,1,3);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testNotEquals1() throws LexicalException {
		String input = "!=!=!";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		checkNext(scanner, OP_NEQ, 0,2,1,1);
		checkNext(scanner, OP_NEQ, 2,2,1,3);
		checkNext(scanner, OP_EXCL, 4,1,1,5);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testMinus1() throws LexicalException {
		String input = "--->--";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		checkNext(scanner, OP_MINUS, 0,1,1,1);
		checkNext(scanner, OP_MINUS, 1,1,1,2);
		checkNext(scanner, OP_RARROW, 2,2,1,3);
		checkNext(scanner, OP_MINUS, 4,1,1,5);
		checkNext(scanner, OP_MINUS, 5,1,1,6);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testEquals3() throws LexicalException {
		String input = "===";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		checkNext(scanner, OP_EQ, 0,2,1,1);
		checkNext(scanner, OP_ASSIGN, 2,1,1,3);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testEquals4() throws LexicalException {
		String input = "====";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		checkNext(scanner, OP_EQ, 0,2,1,1);
		checkNext(scanner, OP_EQ, 2,2,1,3);
		checkNextIsEOF(scanner);
	}
	
	
	
	@Test
	public void testLessThan1() throws LexicalException {
		String input = "<<=-<-";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		checkNext(scanner, OP_LT, 0,1,1,1);
		checkNext(scanner, OP_LE, 1,2,1,2);
		checkNext(scanner, OP_MINUS, 3,1,1,4);
		checkNext(scanner, OP_LARROW, 4,2,1,5);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testGreaterThan1() throws LexicalException {
		String input = "><=>=<-";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		checkNext(scanner, OP_GT, 0,1,1,1);
		checkNext(scanner, OP_LE, 1,2,1,2);
		checkNext(scanner, OP_GE, 3,2,1,4);
		checkNext(scanner, OP_LARROW, 5,2,1,6);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testAsterisk() throws LexicalException {
		String input = "*****";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		checkNext(scanner, OP_POWER, 0,2,1,1);
		checkNext(scanner, OP_POWER, 2,2,1,3);
		checkNext(scanner, OP_TIMES, 4,1,1,5);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testIntegerLiteral() throws LexicalException {
		//INTEGER_LITERAL
		String input = "->123;;6789";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		checkNext(scanner, OP_RARROW, 0,2,1,1);
		checkNext(scanner, INTEGER_LITERAL, 2,3,1,3);
		checkNext(scanner, SEMI, 5,1,1,6);
		checkNext(scanner, SEMI, 6,1,1,7);
		checkNext(scanner, INTEGER_LITERAL, 7,4,1,8);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testIntegerLiteral2() throws LexicalException {
		//INTEGER_LITERAL
		String input = "->010203;;06789";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		checkNext(scanner, OP_RARROW, 0,2,1,1);
		checkNext(scanner, INTEGER_LITERAL, 2,1,1,3);
		checkNext(scanner, INTEGER_LITERAL, 3,5,1,4);
		checkNext(scanner, SEMI, 8,1,1,9);
		checkNext(scanner, SEMI, 9,1,1,10);
		checkNext(scanner, INTEGER_LITERAL, 10,1,1,11);
		checkNext(scanner, INTEGER_LITERAL, 11,4,1,12);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void failIntegerLiteral1() throws LexicalException {
		String input = "1234567899999";
		show(input);
		
		thrown.expect(LexicalException.class);  //Tell JUnit to expect a LexicalException
		try {
			 new Scanner(input).scan();
		} catch (LexicalException e) {  //
			show(e);
			assertEquals(0,e.getPos());
			throw e;
		}
	
	}
	
	
	@Test
	public void failIntegerLiteral2() throws LexicalException {
		String input = "->;==1234567899999";
		show(input);
		thrown.expect(LexicalException.class);  //Tell JUnit to expect a LexicalException
		try {
			new Scanner(input).scan();
		} catch (LexicalException e) {  //
			show(e);
			assertEquals(5,e.getPos());
			throw e;
		}
	}
	
	@Test
	public void testBooleanLiteral1() throws LexicalException {
		//BOOLEAN_LITERAL
		String input = "trueandfalse;false";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		checkNext(scanner, IDENTIFIER, 0,12,1,1);
		checkNext(scanner, SEMI, 12,1,1,13);
		checkNext(scanner, BOOLEAN_LITERAL, 13,5,1,14);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testBooleanLiteral2() throws LexicalException {
		//BOOLEAN_LITERAL
		String input = "true";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		checkNext(scanner, BOOLEAN_LITERAL, 0,4,1,1);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testDivision1() throws LexicalException {
		String input = "12/;";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		checkNext(scanner, INTEGER_LITERAL, 0,2,1,1);
		checkNext(scanner, OP_DIV, 2,1,1,3);
		checkNext(scanner, SEMI, 3,1,1,4);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testComment1() throws LexicalException {
		String input = "abc//mn12\nrtq";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		checkNext(scanner, IDENTIFIER, 0,3,1,1);
		checkNext(scanner, IDENTIFIER, 10,3,2,1);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testComment2() throws LexicalException {
		String input = "//123\n_abc1";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		checkNext(scanner, IDENTIFIER, 6,5,2,1);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testComment4() throws LexicalException { 
		String input = "123\tabc";
		System.out.println(input);
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		checkNext(scanner, INTEGER_LITERAL, 0,3,1,1);
		checkNext(scanner, IDENTIFIER, 4,3,1,5);
		checkNextIsEOF(scanner);
		
	}

	@Test
	public void testReservedWords() throws LexicalException { 
		String input = "SCREEN cart_x _xy_1";
		System.out.println(input);
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		checkNext(scanner, KW_SCREEN, 0,6,1,1);
		checkNext(scanner, KW_cart_x, 7,6,1,8);
		checkNext(scanner, IDENTIFIER, 14,5,1,15);
		checkNextIsEOF(scanner);
		
	}

	@Test
	public void testReservedBooleanLiteral() throws LexicalException {
		//BOOLEAN_LITERAL and Reserved
		String input = "x:a;Abc;_45";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		checkNext(scanner, KW_x, 0,1,1,1);
		checkNext(scanner, OP_COLON, 1,1,1,2);
		checkNext(scanner, KW_a, 2,1,1,3);
		checkNext(scanner, SEMI, 3,1,1,4);
		checkNext(scanner, IDENTIFIER, 4,3,1,5);
		checkNext(scanner, SEMI, 7,1,1,8);
		checkNext(scanner, IDENTIFIER, 8,3,1,9);	
		checkNextIsEOF(scanner);
	}

	@Test
	public void testStringLiteral() throws LexicalException {
		String input = "tb\"abc\"123";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		checkNext(scanner, IDENTIFIER, 0,2,1,1);
		checkNext(scanner, STRING_LITERAL, 2,5,1,3);
		checkNext(scanner, INTEGER_LITERAL, 7,3,1,8);	
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testLiterals() throws LexicalException {
		String input = "pt \"mango\" \n100";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		checkNext(scanner, IDENTIFIER, 0,2,1,1);
		checkNext(scanner, STRING_LITERAL, 3,7,1,4);
		checkNext(scanner, INTEGER_LITERAL, 12,3,2,1);	
		checkNextIsEOF(scanner);
	}
	

	@Test
	public void testSemi() throws LexicalException {
		String input = ";;\n;;";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		checkNext(scanner, SEMI, 0, 1, 1, 1);
		checkNext(scanner, SEMI, 1, 1, 1, 2);
		checkNext(scanner, SEMI, 3, 1, 2, 1);
		checkNext(scanner, SEMI, 4, 1, 2, 2);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testSpaces() throws LexicalException {
		String input = "abcd pqrs";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		checkNext(scanner, IDENTIFIER, 0, 4, 1, 1);
		checkNext(scanner, IDENTIFIER, 5, 4, 1, 6);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void failInvalidInput2() throws LexicalException {
		String input = "`";
		show(input);
		thrown.expect(LexicalException.class);  //Tell JUnit to expect a LexicalException
		try {
			new Scanner(input).scan();
		} catch (LexicalException e) {  //
			show(e);
			assertEquals(0,e.getPos());
			throw e;
		}
	}
	
	@Test
	public void failInvalidInput3() throws LexicalException {
		String input = "\'";
		show(input);
		thrown.expect(LexicalException.class);  //Tell JUnit to expect a LexicalException
		try {
			new Scanner(input).scan();
		} catch (LexicalException e) {  //
			show(e);
			assertEquals(0,e.getPos());
			throw e;
		}
	}
	
	@Test
	public void testComment5() throws LexicalException {
		String input = "//I am bored \r\nGators";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		checkNext(scanner, IDENTIFIER, 15, 6, 2, 1);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testComment6() throws LexicalException {
		String input = "//I am bored \r\nHop\n\r\nGators";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		checkNext(scanner, IDENTIFIER, 15, 3, 2, 1);
		checkNext(scanner, IDENTIFIER, 21, 6, 4, 1);
		checkNextIsEOF(scanner);
	}

	@Test
	public void testHorizonatalTab() throws LexicalException {
		String input = "I like\tgators\fmango ";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		checkNext(scanner, IDENTIFIER, 0, 1, 1, 1);
		checkNext(scanner, IDENTIFIER, 2, 4, 1, 3);
		checkNext(scanner, IDENTIFIER, 7, 6, 1, 8);
		checkNext(scanner, IDENTIFIER, 14, 5, 1, 15);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testStringLiteral2() throws LexicalException {
		String input = "love\"mangoes\b\"";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		checkNext(scanner, IDENTIFIER, 0, 4, 1, 1);
		checkNext(scanner, STRING_LITERAL, 4, 10, 1, 5);
		checkNextIsEOF(scanner);
	}

	/**
	 * This example shows how to test that your scanner is behaving when the
	 * input is illegal.  In this case, we are giving it a String literal
	 * that is missing the closing ".  
	 * 
	 * Note that the outer pair of quotation marks delineate the String literal
	 * in this test program that provides the input to our Scanner.  The quotation
	 * mark that is actually included in the input must be escaped, \".
	 * 
	 * The example shows catching the exception that is thrown by the scanner,
	 * looking at it, and checking its contents before rethrowing it.  If caught
	 * but not rethrown, then JUnit won't get the exception and the test will fail.  
	 * 
	 * The test will work without putting the try-catch block around 
	 * new Scanner(input).scan(); but then you won't be able to check 
	 * or display the thrown exception.
	 * 
	 * @throws LexicalException
	 */
	
	
	@Test
	public void failUnclosedStringLiteral() throws LexicalException {
		String input = "\" greetings  ";
		show(input);
		thrown.expect(LexicalException.class);  //Tell JUnit to expect a LexicalException
		try {
			new Scanner(input).scan();
		} catch (LexicalException e) {  //
			show(e);
			assertEquals(13,e.getPos());
			throw e;
		}
	}
	
	
	@Test
    public void fail() throws LexicalException{
		String input = "prog int x_value=10; int y_value =20; int sinValue; int cosValue;"
 			    + " sinValue=polar_a[x_value*1/2+12+13,y_value*1/3*1/2*3/4%2];"
 		        + " cosValue=cart_y[x_value/2*2+2,(x_value>y_value)?sinValue/234:sinValue*10-23];"
 		        + " int cotValue=polar_r[sinValue/cosValue,sinValue*100/400+cosValue];";
    	show(input);
    	Scanner scanner = new Scanner(input).scan();
    	show(scanner);
    }
	
}
